...
ZAKB address discovery now reads `waste/address-inventory` from the database. The `osm-regional-addresses` collector downloads Geofabrik's Hessen PBF weekly into a temporary file and extracts address-tagged nodes and ways for the configured municipalities/bounding box in two streaming passes. Only relevant way-node coordinates are retained in memory. The DB stores the regional extraction, source URL and full-download SHA-256; the statewide PBF is deleted after processing. Inventories expire after 14 days. This avoids a runtime dependency on public Overpass servers, whose connections failed from the deployed VPS. Failed refreshes preserve the previous publication until its expiry. This remains representative street/address coverage, not a complete address register or observed waste-vehicle tracking.
