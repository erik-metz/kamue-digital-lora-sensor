import { env } from "@/env";
import { toStationNodes, mergeDefaultCrossings, type ApiMapSensor } from "./mapData";
...
export async function fetchMapData() {
  ...
  if (response.status === 404) {
    ...
    return { nodes: mergeDefaultCrossings(toStationNodes(sensors)), readingsAvailable: false };
  }
  if (!response.ok) throw new Error("Map data unavailable");
  const body: { sensors: ApiMapSensor[] } = await response.json();
  return { nodes: mergeDefaultCrossings(toStationNodes(body.sensors)), readingsAvailable: true };
}
