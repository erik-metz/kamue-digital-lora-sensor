import { env } from "@/env";
import { toMapNodes, type ApiMapSensor } from "./mapData";
...
export async function fetchMapData() {
  ...
  if (response.status === 404) {
    ...
    return { nodes: toMapNodes(sensors), readingsAvailable: false };
  }
  if (!response.ok) throw new Error("Map data unavailable");
  const body: { sensors: ApiMapSensor[] } = await response.json();
  return { nodes: toMapNodes(body.sensors), readingsAvailable: true };
}
