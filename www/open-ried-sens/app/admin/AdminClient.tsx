...
export default function AdminClient({
  ...
  async function toggleVisibility(sensor: SensorItem) {
    ...
    try {
      const result = await callAction(() =>
        setVisibilityAction(sensor.sensor_id, !sensor.is_hidden)
      );
      if (result.ok) {
  ...
  async function handleEditSensor(e: React.FormEvent) {
    ...
    try {
      const result = await callAction(() =>
        updateSensorAction(selectedSensor.sensor_id, payload)
      );
      if (result.ok) {
        const updated = result.data;
        setSensors((prev) =>
  ...
  async function handleDeleteSensor(purgeTelemetry: boolean) {
    ...
    try {
      const result = await callAction(() =>
        deleteSensorAction(selectedSensor.sensor_id, purgeTelemetry, deletePassword)
      );
      if (result.ok) {
        if (purgeTelemetry) {
          setSensors((prev) =>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-6">
            ...
            <form onSubmit={handleCreateSensor} className="space-y-4">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-slate-500">Buchstaben, Zahlen, Bindestriche und Unterstriche; maximal 64 Zeichen.</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
                  Bezeichnung / Name *
